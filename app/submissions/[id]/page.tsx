...
export default function SubmissionDetailPage() {
  ...
  } = useSubmissionAPI(params.id as string);
  const { rounds: rawRounds, refetch: refetchRounds } = useReviewRoundsAPI(
    params.id as string
  );
  ...
  const [selectedReviewerByRound, setSelectedReviewerByRound] = useState<
  ...
  const [assigningByRound, setAssigningByRound] = useState<
  ...
  const handlePostDiscussion = useCallback(async () => {
    ...
    try {
      ...
      const refreshed = await apiGet<any[]>(
        `/api/discussions?submissionId=${params.id}`
      );
    ...
    async (roundId?: string) => {
      ...
      if (!rounds || rounds.length === 0) {
        try {
          ...
          const latestRounds = await apiGet<any[]>(
            `/api/reviews/rounds?submissionId=${submission?.id}`
          );
          ...
          if (currentRound?.id) {
            ...
            setSelectedReviewerByRound((prev) => ({
              ...prev,
      ...
      try {
        const currentRound =
          rounds.find((r: any) => String(r?.id) === rid) ||
        ...
        const reviewsData = await apiGet<any[]>(
          `/api/reviews?submissionId=${submission?.id}`
        );
  ...
  const handleDecision = useCallback(async () => {
    ...
    try {
      ...
      const currentRound =
        rounds && rounds.length > 0 ? rounds[rounds.length - 1].id : null;
  ...
  const handleUploadRevisionFile = useCallback(
    async (file: File) => {
  ...
  const handleWithdraw = useCallback(async (reason?: string) => {
    ...
    setWithdrawing(true);
    try {
      await apiPost(`/api/submissions/${params.id}/withdraw`, {
        reason: reason || "No reason provided"
      });
      toast.success("Submission withdrawn successfully");
      setWithdrawDialog(false);

      // Redirect to submissions list after short delay
      setTimeout(() => {
    ...
    const normalized = files
      .map((f: any) => {
        ...
        const stage =
          ...
          "submission";
        const date =
          f?.date_uploaded || f?.uploadedAt || f?.dateUploaded || null;
        const ts = date ? new Date(date).getTime() : 0;
        return {
          ...f,
          __displayName: String(name),
          __displayStage: String(stage),
          __displayTs: ts,
        };
      })
  ...
  const filesForDisplay = showAllFileVersions
    ? Array.isArray(files)
      ? files
      : []
    : displayFiles;
  ...
  const fileStageLabel = (stageRaw: any) => {
    ...
    if (s.includes("production") || s.includes("proof") || s.includes("galley"))
      return "Production";
  ...
  const groupedFiles = (() => {
    ...
    return order
      .map((k) => ({
        key: k,
        items: Array.isArray(groups[k]) ? groups[k] : [],
      }))
  ...
  const computedCurrentRound = (() => {
    const direct =
      (submission as any)?.currentRound ?? (submission as any)?.current_round;
    if (
      direct !== undefined &&
      direct !== null &&
      String(direct) !== "" &&
            ...
            {isRevisionRequired && !isSubmitter && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Upload your revised manuscript and resubmit for review.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  {latestDecision && (
                    <div className="rounded-lg border p-3">
                      <p className="text-sm font-medium">
                        Latest Editor Decision
                      </p>
                      <p className="text-xs text-muted-foreground">
                        {latestDecision?.date_decided
                          ? new Date(
                            latestDecision.date_decided
                          ).toLocaleString()
                          : ""}
                      ...
                      <p className="text-sm mt-2 whitespace-pre-wrap">
                        {latestDecision?.decision_comments ||
                  ...
                  <div className="border-2 border-dashed rounded-lg p-6 text-center">
                    ...
                    <p className="text-sm text-muted-foreground mb-3">
                      Upload revised manuscript (PDF/DOCX)
                    </p>
                    <Button
                      disabled={revisionUploadLoading}
                      onClick={() =>
                        document.getElementById("revision-upload")?.click()
                      }
                    >
              ...
              <TabsContent value="files">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      All files associated with this submission
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    {loadingFiles ? (
                      ...
                      <div className="space-y-4">
                        <div className="flex flex-col gap-2 rounded-lg border p-3 sm:flex-row sm:items-center sm:justify-between">
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">
                              When off, only the latest file per stage and
                              filename is shown
                            </p>
                          </div>
                          <Switch
                            checked={showAllFileVersions}
                            onCheckedChange={setShowAllFileVersions}
                          />
                        </div>

                        <Accordion
                          type="multiple"
                          defaultValue={groupedFiles.map((g) => g.key)}
                          className="space-y-2"
                        >
              ...
              <TabsContent value="participants">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Authors and reviewers involved in this submission
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    <div>
                        ...
                        <div className="space-y-2">
                          {reviews.map((review: any, idx: number) => {
                            ...
                            const reviewer =
                              ...
                              (reviewers || []).find(
                                (r: User) =>
                                  String(r.id) === String(review?.reviewerId)
                              );
                            const displayName =
                              `${reviewer?.firstName ||
                                reviewer?.first_name ||
                                ""
                                } ${reviewer?.lastName || reviewer?.last_name || ""
                                }`.trim() ||
                              reviewer?.email ||
                              review?.reviewerId ||
                              "Reviewer";
                              ...
                              >
                                <div className="flex items-center gap-3 min-w-0">
                                  <Avatar className="h-8 w-8">
                                    <AvatarFallback className="text-xs">
                                      {(
                                        String(displayName)[0] || "R"
                                      ).toUpperCase()}
                                      {(
                                        String(displayName)[1] || "E"
                                      ).toUpperCase()}
                                    </AvatarFallback>
                                  </Avatar>
                ...
                {!rounds || rounds.length === 0 ? (
                  <Card>
                    <CardContent className="flex flex-col items-center justify-center py-8">
                      ...
                      <p className="text-sm text-muted-foreground">
                        No review rounds initiated
                      </p>
                      {canSendToReview && (
                        <Button
                          onClick={handleSendToReview}
                          className="mt-4"
                          disabled={sendToReviewLoading}
                        >
                  ...
                  <div className="space-y-4">
                    {rounds.map((round: any) => {
                      const roundReviews = (reviews || []).filter(
                        (r: any) => r?.reviewRoundId === round?.id
                      );
                      return (
                          ...
                          <CardHeader>
                            <div className="flex items-center justify-between">
                              ...
                              <Badge variant="outline">
                                {roundReviews.length} reviewer
                                {roundReviews.length !== 1 ? "s" : ""}
                              </Badge>
                            </div>
                            {round?.dateCreated && (
                              <CardDescription>
                              ...
                              <div className="flex items-center gap-2 pt-2">
                                      ...
                                      if (available.length === 0) {
                                        return (
                                          <div className="p-3 text-sm text-muted-foreground">
                                            No available reviewers. Add another
                                            reviewer account or use a different
                                            round.
                                          </div>
                                        );
                                      }
                                ...
                                <Button
                                  ...
                                  disabled={
                                    ...
                                    !!assigningByRound[String(round?.id || "")]
                                  }
                                >
              ...
              <TabsContent value="discussion">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Internal discussion for this submission
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <div className="flex justify-end">
                        <Button
                          ...
                          disabled={
                            discussionPosting || !discussionMessage.trim()
                          }
                        >
                      ...
                      <div className="space-y-3">
                        {discussions.map((d: any, idx: number) => {
                          ...
                          const createdAt = createdAtRaw
                            ? new Date(
                              String(createdAtRaw).includes("T")
                                ? String(createdAtRaw)
                          ...
                          return (
                            <div
                              key={d?.id || d?.discussion_id || `d-${idx}`}
                    ...
                    <div className="space-y-4">
                      ...
                      {(rounds || []).map((round: any) => (
                        ...
                        >
                          ...
                          <div className="flex-1 pb-4">
                            <p className="text-sm font-medium">
                              Review Round {round?.round || "?"} Started
                            </p>
                            {round?.dateCreated && (
                              <p className="text-xs text-muted-foreground">
                                {format(
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span>
                    {(submission as any)?.section?.title ||
                      (submission as any)?.sections?.[0]?.title ||
                      "Unknown"}
                  </span>
                </div>
              </CardContent>
