...
import type { AssignReviewerRequest } from "@/lib/types/workflow"
import { WorkflowStage, ReviewMethod } from "@/lib/types/workflow"
...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        const body: AssignReviewerRequest = await request.json()

        const {
            submissionId,
            reviewerId,
            reviewRoundId,
            stageId = WorkflowStage.EXTERNAL_REVIEW,
            reviewMethod = ReviewMethod.BLIND,
            dateDue,
            dateResponseDue
        } = body
        ...
        if (submissionError || !submission) {
            logger.apiError('/api/reviews/assign', 'POST',
                `Submission ${submissionId} not found`, user?.id)
        ...
        if (user?.id && submission.submitter_id && String(submission.submitter_id) === String(user.id)) {
            logger.warn('Conflict of interest: editor attempted to assign reviewer to own submission', {
                submissionId,
        ...
        const now = new Date()
        const defaultResponseDue = new Date(now.getTime() + 3 * 24 * 60 * 60 * 1000) // 3 days
      ...
      `)
        ...
        if (assignmentError || !assignment) {
            ...
            return NextResponse.json({
                error: assignmentError?.message || "Failed to create review assignment",
                success: false
            }, { status: 500 })
        }
...
export async function GET(request: NextRequest) {
    try {
        ...
        if (error) {
            ...
            const isRolesTypeIssue = message.toLowerCase().includes('contains') || message.toLowerCase().includes('operator does not exist')
            ...
            if (isRolesTypeIssue) {
                ...
                if (submissionId) {
                    ...
                    if (assigned2 && assigned2.length > 0) {
                        const assignedIds = assigned2
                            ...
                            .map((id: any) => `'${String(id).replace(/'/g, "''")}'`)
                        if (assignedIds.length > 0) {
                            fallbackQuery = fallbackQuery.not("id", "in", `(${assignedIds.join(',')})`)
                        }
                    }
                }
                ...
                const transformedFallback = transformFromDB((fallbackResp.data || []).map((u: any) => {
                    const { roles, ...rest } = u || {}
                    return rest
                }))
            ...
            if (!hasServiceRole && code === '42501') {
                return NextResponse.json({
                    data: [],
                    success: true,
                    warning: 'RLS prevented listing reviewers. Configure SUPABASE_SERVICE_ROLE_KEY for editor reviewer search.'
                })
            }
        }

        const transformed = transformFromDB((reviewers || []).map((u: any) => {
            const { roles, ...rest } = u || {}
            return rest
        }))

        const warning = (!hasServiceRole && Array.isArray(transformed) && transformed.length === 0)
        ...
        if (warning) {
            logger.warn('Reviewer list empty (possible RLS)', {
                submissionId,
                search,
                hasServiceRole,
            }, { userId: user?.id, route: '/api/reviews/assign' })
