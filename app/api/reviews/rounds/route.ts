...
export async function POST(request: NextRequest) {
  try {
    ...
    const submissionIdStr = typeof data.submissionId === 'number' ? String(data.submissionId) : data.submissionId
    ...
    if (submissionError) {
      console.error("[Review Rounds POST] Submission query error:", submissionError)
      return NextResponse.json(
    ...
    if (lastRound && Number(lastRound.status) !== REVIEW_ROUND_STATUS_REVIEWS_COMPLETED) {
      return NextResponse.json(
        {
          ...
          dateCreated: (lastRound as any).created_at || (lastRound as any).date_created || null,
          existing: true,
        },
        { status: 200 }
      )
    ...
    if (createRoundError) {
      console.error("[Review Rounds POST] Create round error:", createRoundError)
      return NextResponse.json({ error: createRoundError.message }, { status: 500 })
    }
    ...
    if (updateAttempt.error) {
      console.warn("[Review Rounds POST] Submission update failed:", updateAttempt.error.message)
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (submissionError) {
      console.error('[Review Rounds GET] Submission query error:', submissionError)
    ...
    console.log('[Review Rounds GET] Success:', { submissionId, roundsCount: enriched.length })
    return NextResponse.json(enriched)
  } catch (error: any) {
    console.error("Get review rounds error:", error)
    return NextResponse.json({ error: "Failed to fetch review rounds" }, { status: 500 })
  }
}
